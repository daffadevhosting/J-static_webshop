...
var cart_kosong = '<div class="empty-cart"><svg xmlns="http://www.w3.org/2000/svg" width="140" height="140" fill="currentColor" class="bi bi-basket2-fill" viewBox="0 0 16 16"><path d="M5.929 1.757a.5.5 0 1 0-.858-.514L2.217 6H.5a.5.5 0 0 0-.5.5v1a.5.5 0 0 0 .5.5h.623l1.844 6.456A.75.75 0 0 0 3.69 15h8.622a.75.75 0 0 0 .722-.544L14.877 8h.623a.5.5 0 0 0 .5-.5v-1a.5.5 0 0 0-.5-.5h-1.717L10.93 1.243a.5.5 0 1 0-.858.514L12.617 6H3.383zM4 10a1 1 0 0 1 2 0v2a1 1 0 1 1-2 0zm3 0a1 1 0 0 1 2 0v2a1 1 0 1 1-2 0zm4-1a1 1 0 0 1 1 1v2a1 1 0 1 1-2 0v-2a1 1 0 0 1 1-1"/></svg><p class="center">Keranjang Masih Kosong</p></div>';
        ...
        generateSimpleCart = function (space) {
            ...
            cartColumnViews = {
                ...
                ukuran: function (item, column) {
                    return ["<span class='ukuran'>Ukuran :</span> " + item.get(column.attr)];
                },
            ...
            simpleCart.extend({
                ...
                writeCart: function (selector) {
                    var TABLE = settings.cartStyle.toLowerCase(),
                        ...
                        x,
                        xlen;
                       $(".ck-btn,.basket-detail,.harga-total,#total-harga,.checkout-layout").hide();
                       $(".keranjang-kosong,.product-btn").show();
                ...
                createCartRow: function (item, y, TR, TD, container) {
                    var row = simpleCart
                        ...
                        content,
                        cell;
                    $(".empty-cart,.keranjang-kosong,.product-btn").hide();
                    $(".ck-btn,.basket-detail,.harga-total,#total-harga,.checkout-layout").show();
...
simpleCart.extend({
    checkoutWithMidtrans: function () {
        ...
        // Mengumpulkan item dalam keranjang
        simpleCart.each(function (item, x) {
            var counter = x + 1;
            data.items.push({
                id: item.id(),
        ...
        // Mengirim data ke server Anda untuk mendapatkan token transaksi dari Midtrans
        $.ajax({
            url: '/create-midtrans-transaction', // endpoint server Anda untuk membuat transaksi
            method: 'POST',
            contentType: 'application/json',
            data: JSON.stringify(data),
            success: function (response) {
                // Mendapatkan token transaksi dan memanggil Snap untuk menampilkan UI pembayaran
...
// Menghubungkan fungsi checkout dengan tombol checkout
simpleCart.ready(function () {
    simpleCart.bindInputs([
        {
            selector: "checkout",
            event: "click",
            callback: function () {
                simpleCart.checkoutWithMidtrans();
            }
        }
    ]);
});
...
(function() {
    // --- Konfigurasi ---
    var dev = 'Si1zdGF0aWM=';                // base64 dari "J-static"
    var myLicense = atob(dev);
    var redirectUrl = 'https://daffadevhosting.github.io/';
    var countdownSeconds = 10;
    var metaTag = document.querySelector('meta[name="license"]');
    var metaContent = metaTag ? metaTag.getAttribute('content') : '';

    // --- Jika lisensi valid, tidak lakukan apa-apa ---
    if (metaContent === myLicense) {
        return;   // lisensi cocok, halaman berjalan normal
    }

    // --- Lisensi tidak valid → tampilkan peringatan ---
    var style = document.createElement('style');
    style.textContent = `
        body { background: #000000b3 !important; overflow: hidden; }
        #peringatan {
        ...
        @media only screen and (max-width: 680px) {
            ...
            #peringatan h4 { font-size: x-large !important; }
        }
    `;
    document.head.appendChild(style);

    var warningDiv = document.createElement('div');
    warningDiv.id = 'peringatan';
    warningDiv.innerHTML = `
        <h4 style="margin-bottom: 35px; font-size:-webkit-xxx-large;">🔒︄ Template is Locked Up</h4>
        <p style="margin-top: 20px; font-size: 18px; letter-spacing: 2px; line-height: 30px;">
            Mohon untuk tidak merubah link footer.
        </p>
        <p>Redirecting in <span id="aktivasi">${countdownSeconds}</span> seconds...</p>
    `;
    ...
    var counter = setInterval(function() {
        remaining--;
        var countdownSpan = document.getElementById('aktivasi');
